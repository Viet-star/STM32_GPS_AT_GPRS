...
*
*  @author xuanviet , mình đang là 1 embedded software engineer  tại CMC coporation 
*  @bug issue #9
*/
#include <string.h>
#include <stdio.h>
...
uint8_t sim_insert_PIN(char * pin){
	...
	char PIN_insert_cmd[13]= "AT+CFUN=";
	uint8_t PIN_status=0;
	char local_rx_buffer[RX_BUFFER_LENGTH]; 
	
	/* xây dựng chuỗi PIN_insert_cmd = AT+CFUN=XXXX   Độ dài SIM_PIN được giả định = 4 */
	strcat(PIN_insert_cmd,PASSWORD);   
	
	/* xây dựng chuỗi  PIN_insert_cmd = AT+CFUN=XXXX\r */
...
/**
 * cho phép kết nối GPRS. GPRS phải được bật trước khi cố gắng thiết lập kết nối TCP. Chức năng này trải qua một loạt các thử nghiệm dưới đây. hành động được thực hiện tùy thuộc vào kết quả thử nghiệm

   	Kiểm tra xem thẻ SIM có được phát hiện không
	Kiểm tra xem thẻ SIM có yêu cầu mã PIN không
	Kiểm tra cường độ tín hiệu. Nếu cường độ tín hiệu quá yếu, nó sẽ trả về giá trị lỗi.
	Kiểm tra xem Thiết bị di động (ME) có được đăng ký vào Mạng không. Nếu không, nó sẽ cố gắng đăng ký.
	Kiểm tra xem ME có được kết nối với dịch vụ GPRS không. Nếu chưa, nó sẽ thử kết nối.
	Kiểm tra xem ngữ cảnh GPRS PDP đã được xác định hay chưa, nếu chưa, nó sẽ thử xác định, kích hoạt và lấy địa chỉ IP.
 * @return  nếu gprs đang hoạt động, 2 nếu thẻ SIM không được phát hiện, 3 nếu mã PIN SIM không đúng, 4 nếu tín hiệu yếu, 0 nếu không
 */
uint8_t enable_gprs(){
	...
	#endif
	/* xóa bộ nhớ đệm sau mỗi lần sử dụng */
	char local_rx_buffer[RX_BUFFER_LENGTH]; 
	char local_rx_buffer[RX_BUFFER_WIDTH];


	/* Một số thử nghiệm được thực hiện 3 lần, trial_counter theo dõi số lượng thử nghiệm đã diễn ra */
	uint8_t trials_counter=0;
	
	
	
	/*** Kiểm tra xem chức năng điện thoại của mô-đun có được bật hay không ***/
	...
	while(trials_counter <3){
		...
		/* Kiểm tra phản hồi của mô-đun trong local_rx_buffer để xem điện thoại có được bật hay không.
		 * Lưu trạng thái trong phone_status
		 */
	
		#ifdef DEBUG_MODE
			send_debug("Kiểm tra xem chức năng điện thoại của mô-đun có được bật hay không: send AT+CFUN?");
		#endif
		...
		memset(local_rx_buffer,NULL,RX_BUFFER_LENGTH);
			...
			if (is_phone_enabled==1)
				...
				if (trials_counter>=3)
					...
					{
						#ifdef DEBUG_MODE
							send_debug("Kích hoạt chức năng điện thoại: send AT+CFUN=1");
						#endif
	...
	/*** Phát hiện xem có thẻ SIM không ***/
	...
	/*** Kiểm tra xem có cần mã PIN không ***/
	...
	#ifdef DEBUG_MODE
		send_debug("gửi lệnh tới Kiểm tra xem có cần mã PIN không: send AT+CPIN?");
	#endif
	send_AT_cmd(PIN_status_cmd,"OK",1,local_rx_buffer,RX_TIMEOUT);
	
	/* Note: mô-đun trả lời SẴN SÀNG nếu không cần mã PIN*/
	/* Cần mã PIN thì nhập mã PIN, sai mã PIN thì thoát*/
